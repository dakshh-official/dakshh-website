...
const DialCarousel = ({ events, activeId }: DialCarouselProps) => {
  ...
  useEffect(() => {
    ...
    const activeBtn = mobileScrollRef.current.querySelector(
      '[data-active="true"]',
    ) as HTMLElement;
    if (activeBtn) {
  ...
  const normalizedActiveIndex =
    len > 0 ? ((visualActiveIndex % len) + len) % len : 0;
    ...
    <>
          ...
          {events.map((event, index) => {
            ...
            return (
              <motion.button
                ...
                transformTemplate={({ rotate, x, scale }) =>
                  `rotate(${rotate}) translateX(${x}) scale(${scale})`
                }
                onClick={() => {
                ...
                onMouseLeave={() =>
                  setHoveredDialId((prev) => (prev === event._id ? null : prev))
                }
              >
                <div
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 sm:hidden">
        ...
        <div className="relative">
          <div className="absolute left-0 top-0 bottom-0 w-8 bg-linear-to-r from-black/90 to-transparent z-10 pointer-events-none" />
          <div className="absolute right-0 top-0 bottom-0 w-8 bg-linear-to-l from-black/90 to-transparent z-10 pointer-events-none" />
...
const featuredColor = (cat: string, active: boolean) => {
  if (active)
    return "bg-white text-black border-white shadow-[0_0_20px_rgba(255,255,255,0.8)] z-20 ring-2 ring-black";

  switch (cat) {
    case "Software":
      return "bg-blue-900/90 text-blue-100 border-blue-400";
    case "Hardware":
      return "bg-orange-900/90 text-orange-100 border-orange-400";
    case "Entrepreneurship":
      return "bg-green-900/90 text-green-100 border-green-400";
    case "Gaming":
      return "bg-purple-900/90 text-purple-100 border-purple-400";
    case "Design and Prototyping":
      return "bg-cyan-900/90 text-cyan-100 border-cyan-400";
    default:
      return "bg-gray-900/90 text-gray-100 border-gray-400";
  }
};
...
const mobileColor = (cat: string) => {
  switch (cat) {
    case "Software":
      return "bg-blue-900/80 text-blue-100 border-blue-400/60";
    case "Hardware":
      return "bg-orange-900/80 text-orange-100 border-orange-400/60";
    case "Entrepreneurship":
      return "bg-green-900/80 text-green-100 border-green-400/60";
    case "Gaming":
      return "bg-purple-900/80 text-purple-100 border-purple-400/60";
    case "Design and Prototyping":
      return "bg-cyan-900/80 text-cyan-100 border-cyan-400/60";
    default:
      return "bg-gray-900/80 text-gray-100 border-gray-400/60";
  }
};
